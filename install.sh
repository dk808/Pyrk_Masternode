...
#wallet information
BOOTSTRAP_ZIP='https://www.dropbox.com/s/8e9i40gwx5oow0x/pyrk-bootstrap.zip'
BOOTSTRAP_ZIPFILE='pyrk-bootstrap.zip'
CONFIG_DIR='.pyrk'
...
echo -e "${YELLOW}==========================================================="
echo -e 'PYRK Masternode Setup'
echo -e "===========================================================${NC}"
echo -e "${BLUE}Feb 2021, created by dk808${NC}"
echo -e
...
#functions
function wipe_clean() {
    echo -e "${YELLOW}Removing any instances of Pyrk...${NC}"
...
externalip=$WANIP
addnode=5.189.169.23
addnode=103.244.38.18
addnode=173.199.122.102
addnode=103.244.37.70
addnode=95.217.140.148
addnode=103.244.37.136
addnode=63.209.33.53
addnode=103.244.38.113
addnode=54.36.172.184
addnode=95.217.140.133
addnode=209.151.154.214
addnode=62.171.132.112
maxconnections=256
EOF
}
...
function bootstrap() {
    if whiptail --yesno "Would you like to bootstrap the chain?" 8 42; then
        if [ -d $HOME/$CONFIG_DIR/blocks ]; then
            rm -rf $HOME/$CONFIG_DIR/blocks $HOME/$CONFIG_DIR/chainstate $HOME/$CONFIG_DIR/smsgDB $HOME/$CONFIG_DIR/evodb
...
EOF
        ...
        echo "$SENTINEL_CONF" > $HOME/sentinel/sentinel.conf
        cd
        crontab -l | grep -v "$HOME/sentinel && SENTINEL_DEBUG=1 ./venv/bin/python bin/sentinel.py" | crontab -
        sleep 1
        crontab -l > tempcron
        echo "* * * * * cd $HOME/sentinel && SENTINEL_DEBUG=1 ./venv/bin/python bin/sentinel.py >> sentinel.log 2>&1" >> tempcron
        crontab tempcron
        rm tempcron
    fi
}
...
function create_service() {
    echo -e "${YELLOW}Creating Pyrk service...${NC}"
...
function log_rotate() {
    ...
    sleep 1
    if [ -f /etc/logrotate.d/pyrkdebuglog ]; then
        echo -e "${YELLOW}Existing log rotate conf found, backing up to ~/pyrkdebuglogrotate.old ...${NC}"
        sudo mv /etc/logrotate.d/pyrkdebuglog ~/pyrkdebuglogrotate.old
        sleep 2
    fi
    sudo touch /etc/logrotate.d/pyrkdebuglog
    sudo chown $USERNAME:$USERNAME /etc/logrotate.d/pyrkdebuglog
    cat << EOF > /etc/logrotate.d/pyrkdebuglog
$HOME/.pyrk/debug.log {
  compress
  copytruncate
  missingok
...
EOF
    sudo chown root:root /etc/logrotate.d/pyrkdebuglog
}
...
printf "${BLUE}"
figlet -t -k "PYRK  MASTERNODES" 
printf "${STOP}"

echo -e "${YELLOW}==========================================================================================================="
echo -e "${CYAN}COURTESY OF DK808 FROM ALTTANK${NC}"
echo
